...
export default function ResearchModePage() {
  ...
  const generateReportForRegion = (locKey: string, isAuto = false) => {
    ...
    setTimeout(() => {
      ...
      const report: ResearchReport = {
        reportId: `ORCA-RES-${locKey.toUpperCase()}-${Date.now().toString().slice(-6)}`,
  ...
  const handleExportCSV = () => {
    ...
    const rows = [
      ...
      ["Header", "Report ID", currentReport.reportId, "ID", "Generated by ORCA Multi-Agent Network"],
...
Powered by ORCA Multi-Agent Network (CMFRI Baseline Model Active)
    `.trim();
    ...
    <div className="p-6 max-w-7xl mx-auto flex flex-col gap-6 text-slate-800">
      ...
      {currentReport && (
        <div className="bg-white border border-stone-200 rounded-3xl p-6 shadow-sm flex flex-col gap-6">
          ...
          {/* Section 3 Requirement: Powered by ORCA Agent Network tag */}
